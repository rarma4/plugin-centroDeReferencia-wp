<?php
/**
 * Plugin Name: Centro de Referência
 * Plugin URI: https://rafart.com.br/
 * Description: Plugin de Localização dos centros de referências de saúde. Ative o plugin e insira o shortcode <strong>[centroRef]</strong> na página desejada.
 * Author: Rafael Marinho
 * Author URI: https://rafart.com.br/
 * Version: 2.2.0
 *
 */

// Exit if accessed directly.
function centro_scripts_function() {
  wp_enqueue_script( 'scriptCentro', plugins_url() . '/centro-de-referencia/assets/js/scriptCentro.js');
  }
  add_action('wp_enqueue_scripts','centro_scripts_function');

if ( ! defined( 'ABSPATH' ) ) {
	exit;   
}

// echo "<link href='". plugin_dir_url( __FILE__ )."assets/css/style.css' type='text/css' rel='stylesheet' />";

// Register Custom Post Type centros
function custom_post_type_centros() {
  $labels = array(
  'name'                  => 'Centros', 'Post Type General Name',
  'singular_name'         => 'centro', 'Post Type Singular Name',
  'menu_name'             => 'Centros Ref.',
  'name_admin_bar'        => 'centros',
  'archives'              => 'Arquivos de centros',
  'attributes'            => 'Atributos do centro',
  'parent_item_colon'     => 'centro pai',
  'all_items'             => 'Todos os centros',
  'add_new_item'          => 'Adicionar novo centro',
  'add_new'               => 'Adicionar Centro',
  'new_item'              => 'Novo centro',
  'edit_item'             => 'Editar centro',
  'update_item'           => 'Atualizar centro',
  'view_item'             => 'Visualizar centro',
  'view_items'            => 'Visualizar centros',
  'search_items'          => 'Pesquisar centro',
  ...
  'insert_into_item'      => 'Inserir no centro',
  'uploaded_to_this_item' => 'Carregados no centro',
  'items_list'            => 'Lista de centros',
  'items_list_navigation' => 'Lista de navegação',
  'filter_items_list'     => 'Filtrar lista',
  );
  $args = array(
  'label'                 => 'centro',
  'description'           => 'CEntros de referência de saúde',
  'labels'                => $labels,
  'supports'              => array( 'title', 'editor', 'thumbnail', 'comments', 'trackbacks', 'revisions', 'custom-fields', 'post-formats' ),
  'taxonomies'            => array('centro_categories'),
  'hierarchical'          => false,
  'public'                => true,
  'show_ui'               => true,
  'show_in_menu'          => true,
...
add_action( 'init', 'custom_post_type_centros', 0 );

// Register Custom Post Type centros
function centros_taxonomy() {  
  register_taxonomy(  
      'centro_categories',  //The name of the taxonomy. Name should be in slug form (must not contain capital letters or spaces). 
      'centros',        //post type name
      array(  
          'hierarchical' => true,  
          'label' => 'Categoria do centro de referência',  //Display name
          'query_var' => true,
          'rewrite' => array(
              'slug' => 'centros', // This controls the base slug that will display before each term
...
// adicionando os campos personalizados
  ...
  if ( $_GET['post_type'] == 'centros' ) {
  add_post_meta($post_id, 'endereco', '', true);
  add_post_meta($post_id, 'cep', '', true);
  add_post_meta($post_id, 'cidade', '', true);
  add_post_meta($post_id, 'uf', '', true);
  }
  return true;
  }
...
add_action( 'init', 'centros_taxonomy');
...
#svg-map a:active{ fill:#003399 !important }
...
<div class="container">
        <div class="row mt-5">
          <div class="col-sm-12 centralizar">
            <div>
              <h4>Clique em seu Estado para verificar o Centro de Referência</h4>
              <hr>
              </div>
          </div>
          <div class="col-sm-12 centralizar">
            <svg version="1.1" id="svg-map" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="450px" height="460px" viewBox="0 0 450 460" enable-background="new 0 0 450 460" xml:space="preserve">
              <g>
                ...
                <a xlink:href="#santacatarina"id="sc">
                ...
                <a xlink:href="#riograndedosul"id="rs">
                ...
                <a xlink:href="#minasgerais"id="mg">
                ...
                <a xlink:href="#riodejaneiro"id="rj">
                  ...
                  C336.717,337.908,555.927,342.41,332.886,337.429z"></path>
                ...
                <a xlink:href="#espiritosanto"id="es">
                ...
                <a xlink:href="#distritofederal"id="df">
          ...
          <div class="col-sm-12 centralizar">
            <div>
              ...
              <div class="input-group my-5">
                ...
                <select class="form-control" name="estados-brasil" id="uf2">
                ...
                <button type="button" class="btn btn-outline-secondary" id="btnClick"><i class="fa fa-search"></i></button>
              </div>
            </div>
          </div>
        </div>
        <div class="col-sm-12 centralizar"id="lista">
...
// const url = "http://localhost/testewp/wp-content/plugins/centro-de-referencia/assets/centros-de-referencia.json"
const url = "http://localhost/testewp/wp-json/v1/apicentro"
...
add_shortcode('centroRef','centroReferencia');

// echo "<script src='". plugin_dir_url( __FILE__ )."assets/js/js.js'></script>";

//verifica se o plugin ACF esta ativo, caso contrario, pega info do custon fild, se houver
...
function my_api( $req ) {
  // ================================================================================
          ...
          // $item_obj = get_page_by_path( $slug, OBJECT, 'post' );
          // $res = array(
          //     'meu teste' => 'teste ok',
          //     'meu teste2' => 'teste ok mais uma vez'
          // );
  ...
  // ================================================================================
          ...
          if ( $query->have_posts() ): 
              while( $query->have_posts() ): 
                  ...
                  $termos = get_the_terms( $post->ID,'posts' );
                  $termos_resultado = [];
  
                      foreach ($termos as $termo){
                          $termos_resultado[] = $termo->slug;
                      }
                      ...
                      if($acf_plugin == true){
                        ...
                        $item[] = array(
                          'ID'   => $post->ID,
                          'Nome' => get_the_title(),
                          'endereco' => get_post_meta($post->ID, 'endereco', true), //este modelo sera usado para custom fild sem ACF
                          'cep' => get_post_meta($post->ID, 'cep', true), //este modelo sera usado para custom fild sem ACF
                          'cidade' => get_post_meta($post->ID, 'cidade', true), //este modelo sera usado para custom fild sem ACF
                          'UF' => get_post_meta($post->ID, 'uf', true), //este modelo sera usado para custom fild sem ACF
                          'slug' => $post->post_name,
                          'categorias'    => $termos_resultado
          ...
          $res = array( 'posts' => $item );
          return rest_ensure_response( $res );
  
       } 
  
      function centro_api() {
          register_rest_route( 'v1', '/apicentro', array(
              'methods'  => 'GET',
              'callback' => 'my_api'
          ));
      }
  
      add_action( 'rest_api_init', 'centro_api' );
  
      
  // http://localhost/testewp/wp-json/v1/apicentro